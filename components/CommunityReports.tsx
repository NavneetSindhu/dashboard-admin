...
const CommunityReports: React.FC<{ currentTheme: 'light' | 'dark'}> = ({ currentTheme }) => {
    ...
    const handleGenerateInsights = useCallback(async () => {
        ...
        setIsLoading(true);
        setAiInsight('');
        const insight = await generateReportInsights(reports, language);
    ...
    return (
        <div className="space-y-6">
            <div>
                <h1 className="text-3xl font-bold text-gray-800 dark:text-white">{t('community_reports_title')}</h1>
                <p className="text-gray-600 dark:text-gray-400 mt-1">{t('community_reports_desc')}</p>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 flex flex-wrap items-center justify-between gap-4">
                <div className="flex flex-wrap items-center gap-4">
                    <div className="flex-1 min-w-[200px]">
                        ...
                        <select id="location" className="block w-full rounded-lg border-gray-300 bg-gray-50 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200 py-2.5 px-4 focus:border-blue-500 focus:ring-2 focus:ring-blue-200">
                            ...
                            <option>{t('arunachal_pradesh')}</option>
                        </select>
                    </div>
                    <div className="flex-1 min-w-[200px]">
                        <label htmlFor="status" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('status')}</label>
                        <select id="status" className="block w-full rounded-lg border-gray-300 bg-gray-50 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200 py-2.5 px-4 focus:border-blue-500 focus:ring-2 focus:ring-blue-200">
                            <option>{t('all_statuses')}</option>
                            <option>{t('submitted')}</option>
                            <option>{t('reviewed')}</option>
                ...
                <div className="flex items-center gap-x-3 pt-6 sm:pt-0">
                     <button
                        onClick={exportToCSV}
                        className="bg-gray-600 text-white font-semibold px-5 py-2.5 rounded-lg hover:bg-gray-700 dark:bg-gray-600 dark:hover:bg-gray-500 transition-all duration-300 flex items-center shadow-sm"
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead className="bg-gray-50 dark:bg-gray-700">
                        <tr>
                            <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">{t('report_id')}</th>
                            ...
                            <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">{t('date_submitted')}</th>
            ...
            >
                {isLoading ? (
                    <div className="flex flex-col items-center justify-center h-40">
                         ...
                         <p className="mt-4 text-gray-600 dark:text-gray-400">{t('generating_insights_loading')}</p>
                    </div>
                ) : (
                ...
                >
                    <div className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                            <div><strong className="font-semibold text-gray-800 dark:text-gray-200">{t('location')}:</strong><span className="ml-2 text-gray-600 dark:text-gray-400">{selectedReport.location}</span></div>
                            <div><strong className="font-semibold text-gray-800 dark:text-gray-200">{t('date')}:</strong><span className="ml-2 text-gray-600 dark:text-gray-400">{selectedReport.date}</span></div>
                             <div><strong className="font-semibold text-gray-800 dark:text-gray-200">{t('submitted_by')}:</strong><span className="ml-2 text-gray-600 dark:text-gray-400">{selectedReport.submittedBy}</span></div>
                            <div><strong className="font-semibold text-gray-800 dark:text-gray-200">{t('status')}:</strong> <span className={`ml-2 px-2.5 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full ${statusStyles[selectedReport.status]}`}>{selectedReport.status}</span></div>
                            <div><strong className="font-semibold text-gray-800 dark:text-gray-200">{t('patient_age')}:</strong><span className="ml-2 text-gray-600 dark:text-gray-400">{selectedReport.age}</span></div>
                            <div><strong className="font-semibold text-gray-800 dark:text-gray-200">{t('patient_gender')}:</strong><span className="ml-2 text-gray-600 dark:text-gray-400">{selectedReport.gender}</span></div>
                        </div>
                        <div>
                             <strong className="font-semibold text-gray-800 dark:text-gray-200">{t('symptoms_reported')}:</strong>
                             <div className="flex flex-wrap gap-2 mt-2">
                                {selectedReport.symptoms.map(symptom => <span key={symptom} className="bg-gray-200 text-gray-800 dark:bg-gray-700 dark:text-gray-300 text-xs font-medium px-2.5 py-1 rounded-full">{symptom}</span>)}
                             </div>
                        </div>
                         <div>
                            <strong className="font-semibold text-gray-800 dark:text-gray-200">{t('details')}:</strong>
                            <p className="mt-1 text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-700/50 p-3 rounded-lg border dark:border-gray-600">{selectedReport.details}</p>
                        </div>
                         <div className="h-64 rounded-lg overflow-hidden border dark:border-gray-600">
